...
__global__ void func8(unsigned short *r,unsigned char *i,size_t L)
{
    ...
    if(yushu == 0)
    {
     r[index] = (i[index+shang] | (i[index+shang+1] & 0xC0) << 2) / 4;
    }
    else if(yushu == 1)
    {
     r[index] = ((i[index+shang] & 0x3F) | (i[index+shang+1] & 0xF0) << 2) / 4;
    }
    else if(yushu == 2)
    {
     r[index] = ((i[index+shang] & 0x0F) | (i[index+shang+1] & 0xFC) << 2) / 4;
    }
    else if(yushu == 3)
    {
     r[index] = ((i[index+shang] & 0x03) | i[index+shang+1] << 2) / 4;
    }
}
""")    #CUDA函数
...
def conver(fileID):
    ...
    rawFile = filePath + 'RAW_00' + str(fileID) + '.raw10'          #10位拜尔图像文件路径
    ...
    print("Save File " + "DNG_00" + str(fileID) + ".dng")
    RAW2DNG().convert(rawImage, tags=t, filename=("DNG_00"+str(fileID)), path="")
...
#获取像素数据整数值 i = dev.get_attribute(drv.device_attribute.MAX_GRID_DIM_X)
