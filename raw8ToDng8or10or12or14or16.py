...
width = 1920            #图片宽度
height = 1080           #图片高度
bpp= 8   #输出位深只有8、10、12、14和16这5种数值可选  Only 8, 10, 12, 14 and 16 are available
...
class myThread(threading.Thread):
    ...
    def run(self):
        ...
        rawFile = filePath + 'RAW_000' + str(self.fileID) + '.raw8'    #注意文件名要匹配
        rf = open(rawFile, mode='rb')
        rawData = struct.unpack("B"*numPixels,rf.read(numPixels))    #struct.unpack("H"*numPixels,rf.read(2*numPixels))
        ...
        t.set(Tag.WhiteLevel, 255)           #((1 << bpp) -1) 16bit 白场
        ...
        print("Save File " + "DNG_000" + str(self.fileID) + ".dng")
        RAW2DNG().convert(rawImage, tags=t, filename=("DNG_000"+str(self.fileID)), path="")
