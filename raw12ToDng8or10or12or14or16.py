...
class myThread(threading.Thread):
    ...
    def run(self):
        ...
        denominator = 1
        if bpp == 10:
            denominator = 4    #12位转10位需要进行数据转换，否则数据截断
        elif bpp == 8:
            denominator = 16   #12位转8位需要进行数据转换，否则数据截断
        rawFile = filePath + 'RAW_000' + str(self.fileID) + '.raw12'       #注意文件名要匹配
        ...
        while i < len(rawFlatImage):
            if cycle == 0:
                rawFlatImage[i] = (rawData[j] | (rawData[j+1] & 0b11110000) << 4) // denominator
            elif cycle == 1:
                rawFlatImage[i] = ((rawData[j] & 0b00001111) | rawData[j+1] << 4) // denominator
